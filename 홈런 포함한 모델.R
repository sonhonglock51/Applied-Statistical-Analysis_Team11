...
# 3. 변수 선택 & 데이터 분할
# -------------------------------------------------
vars <- c("ra", "slg", "total_hr", "total_bb", "pitch_so", "pitch_bb")
...
# 4. 로지스틱 회귀 모델 학습
# -------------------------------------------------
model <- glm(win_season ~ ra + slg + total_hr + total_bb + pitch_so + pitch_bb,
...
# -------------------------------------------------
new_team <- data.frame(
  ra = 680, slg = 0.415, total_hr = 180,
  total_bb = 520, pitch_so = 1250, pitch_bb = 480
)
